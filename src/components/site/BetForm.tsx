...
import { MATCH, isBettingOpen } from "@/lib/match-config";
...
export function BetForm() {
  ...
  const [scoreBrazil, setScoreBrazil] = useState(0);
  ...
  if (!open) {
    return (
      <div id="aposta" className="rounded-3xl border border-destructive/30 bg-destructive/5 p-8 text-center">
        <h3 className="font-display text-2xl font-bold text-destructive">As apostas foram encerradas.</h3>
        <p className="mt-2 text-sm text-muted-foreground">
          Acompanhe o resultado e o sorteio do prêmio nas nossas redes.
        </p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      >
        <h2 className="font-display text-2xl font-bold sm:text-3xl">Faça sua aposta</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Cada aposta custa <strong className="text-foreground">R$ 20,00</strong> via PIX.
        </p>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <ScoreField label={`Placar ${MATCH.homeTeam}`} value={scoreBrazil} onChange={setScoreBrazil} accent="brasil" />
          <ScoreField label={`Placar ${MATCH.awayTeam}`} value={scoreScotland} onChange={setScoreScotland} accent="escocia" />
        </div>
        ...
        >
          {m.isPending ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" />Gerando PIX…</> : "Gerar PIX — R$ 20,00"}
        </Button>
      </form>
...
}: { label: string; value: number; onChange: (v: number) => void; accent: "brasil" | "escocia" }) {
  const ring = accent === "brasil" ? "focus-within:ring-brasil" : "focus-within:ring-escocia";
  return (
    <div className="space-y-2">
      <Label>{label}</Label>
      <div className={`flex items-center gap-2 rounded-xl border border-input bg-background p-1 ${ring} focus-within:ring-2`}>
